...
";
			...
			using (var f = new TempFile(xml))
			{
				...
				Assert.AreEqual(8, defs.SystemDefines.Count);
				...
				Assert.AreEqual("Peach.JobId", dst[1].Key);
				...
				Assert.AreEqual("Peach/Pits", dst[8].Value);
			}
		}
...
";
			...
			using (var f = new TempFile(xml))
			{
				...
				Assert.AreEqual(10, defs.SystemDefines.Count);
				Assert.AreEqual(1, defs.Platforms.Count);
				...
				Assert.AreEqual("Peach.Plugins", dst[6].Key);
				Assert.AreEqual("Peach.Scripts", dst[7].Key);
				Assert.AreEqual("SamplePath1", dst[8].Key);
				Assert.AreEqual("foo", dst[8].Value);
				Assert.AreEqual("PitLibraryPath", dst[9].Key);
...
";
			...
			using (var f = new TempFile(xml))
			{
				...
				Assert.AreEqual(11, dst.Count);

				Assert.AreEqual("SamplePath", dst[0].Key);
				Assert.AreEqual("bar", dst[0].Value);
				Assert.AreEqual("Executable", dst[1].Key);
				Assert.AreEqual("bar", dst[1].Value);
				Assert.AreEqual("Peach.JobId", dst[2].Key);
				Assert.AreEqual("Peach.OS", dst[3].Key);
				Assert.AreEqual("Peach.Pwd", dst[4].Key);
				Assert.AreEqual("Peach.Cwd", dst[5].Key);
				Assert.AreEqual("Peach.LogRoot", dst[6].Key);
				Assert.AreEqual("Peach.Plugins", dst[7].Key);
				Assert.AreEqual("Peach.Scripts", dst[8].Key);
				Assert.AreEqual("PitLibraryPath", dst[9].Key);
				Assert.AreEqual("Arg", dst[10].Key);
				Assert.AreEqual("bar", dst[10].Value);
			}
		}
