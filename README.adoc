...
NOTE: The maintained build uses SDK-style projects targeting both .NET 8 and
.NET 10 in the original source layout. See link:docs/modern-dotnet.adoc[Modern
.NET build] for supported projects, commands, and remaining compatibility
boundaries.
...
Peach uses SDK-style projects targeting .NET 8 and .NET 10 and supports builds
on Windows, Linux, and macOS. Install a .NET 8 or .NET 10 SDK; `install.sh`
prefers .NET 10 and otherwise falls back to .NET 8. Building every target at
once requires the .NET 10 SDK. Individual integration tests may also need
platform tools such as GDB or Intel Pin.
...
Build the command-line distribution from the repository root with either
supported SDK:
